...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case key.Matches(msg, m.keys.Enter):
			...
			return m.handleButtonPress(button)
		}
	case tea.MouseMsg:
		if msg.Type == tea.MouseLeft {
			for y, row := range m.buttons {
				for x, val := range row {
					// Extremely simplified hit detection
					// Assumes each button is 5 chars wide and 1 char high
					// A real implementation would need to know the absolute position
					// of the buttons in the terminal. This is a placeholder.
...
func (m model) handleButtonPress(button string) (tea.Model, tea.Cmd) {
	m.lastButton = button
	m.isError = false // Reset error on new input
...
func (m model) View() string {
	...
	for y, row := range m.buttons {
		...
		for x, val := range row {
			...
			// Special case for "0" button width
			if val == "0" {
				style = style.Copy().Width(11) // 2 buttons + 1 space
				rowStr = append(rowStr, style.Render(val))
				// Skip the next position since "0" takes up two spots
				if x+1 < len(row) {
					x++
				}
			} else {
	...
	// Help
	b.WriteString("\nUse arrow keys or mouse to navigate. Press Enter to click.\n")
	b.WriteString("Press 'q' or 'esc' to quit.\n")

	return b.String()
}

func isSpecialFunc(s string) bool {
	return s == "AC" || s == "+/-" || s == "%"
}